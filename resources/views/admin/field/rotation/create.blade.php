            ...
            <div class="card-body">
                <form action="{{ route($module . '.store') }}" method="POST">
                    ...
                    <div class="mt-3">
                        ...
                        <input value="{{ old('culture_sort') }}" name="culture_sort" type="text" class="form-control" required>
                    </div>

                    <div class="mt-3">
                        <label class="form-label">Дата посева*</label>
                        <input value="{{ \Carbon\Carbon::parse(old('sowing_date'))->format('Y-m-d') }}" name="sowing_date" type="date" class="form-control" required>
                    </div>

                    <div class="mt-3">
                        <label class="form-label">Дата сбора*</label>
                        <input value="{{ \Carbon\Carbon::parse(old('harvesting_date'))->format('Y-m-d') }}" name="harvesting_date" type="date" class="form-control" required>
                    </div>

                    <div class="mt-3">
                        <label class="form-label">Средний урожай*</label>
                        <input value="{{ old('average_yield') }}" name="average_yield" type="number" class="form-control" required>
                    </div>

                    <div class="mt-3">
                        <label class="form-label">Единица измерения урожая*</label>
                        <input value="{{ old('average_yield_unit') }}" name="average_yield_unit" class="form-control" required>
                    </div>
